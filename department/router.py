...
async def get_all_department(db: AsyncSession = Depends(get_db)) -> list[DepartmentResponse]:
...
async def patch_department(dept_id: int, body: DepartmentPatch, db: AsyncSession = Depends(get_db)):
    id = dept_id
