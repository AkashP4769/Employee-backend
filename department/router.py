...
from department.schemas import DepartmentCreate, DepartmentPatch, DepartmentResponse, GetDepartmentResponse
...
async def get_all_department(db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)) -> list[DepartmentResponse]:
...
async def get_department(dept_id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
    department: Department = await service.get_by_id(db, dept_id=dept_id)

    return department


@router.patch("/{dept_id}", status_code=status.HTTP_200_OK, response_model=DepartmentResponse)
async def patch_department(dept_id: int, body: DepartmentPatch, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
    id = dept_id
...
async def delete_department(dept_id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
