...
import repository.employee_repository as employee_repo
...
async def create(db: AsyncSession, employee: Employee) -> Employee:
    ...
    employee = await employee_repo.create(db, employee=employee)

    return employee


async def get_all_emp(db: AsyncSession) -> list[Employee]:
    db_employees = await employee_repo.get_all(db)

    return db_employees


async def get_employee(db: AsyncSession, user_id: int) -> Employee:
    employee: Employee = await employee_repo.get_employee(db, user_id=user_id)
...
async def patch_employee(db: AsyncSession, employee: Employee) -> Employee:
    original_employee: Employee = await employee_repo.get_employee(db, user_id=employee.id)

    if original_employee is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    
    patched_employee: Employee = await employee_repo.patch_employee(db, original_employee, employee)
...
async def delete_employee(db: AsyncSession, user_id: int) -> Employee:
    ...
    deleted_employee: Employee = await employee_repo.delete_employee(db, employee=employee)
